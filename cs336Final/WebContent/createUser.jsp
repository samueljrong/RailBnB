...
<body>
	<%
		...
		if(userid.isEmpty() || userid.length() > 20) {
			out.println("Invalid username <a href='newUser.jsp'>Try Again</a>");
			return;
		}
		...
		if(userSet.next()){
			out.println("Username already in use. <a href='newUser.jsp'>Try Again</a>");
			return;
		}
		
		// Password Checks
		if(pwd.isEmpty() || pwd.length() > 20) {
			out.println("Invalid password <a href='newUser.jsp'>Try Again</a>");
			return;
		}
		...
		if(!email.isEmpty()) {
			...
			for(int i = 0; i < email.length(); i++){
					if(Character.isWhitespace(email.charAt(i))){
						out.println("Invalid email <a href='newUser.jsp'>Try Again</a>");
						return;
					}
					if(email.charAt(i) == '@'){
						hasAT = true;
		...
		if(!phone.isEmpty()) {
			if(phone.length() != 10){
				out.println("Invalid Phone Number <a href='newUser.jsp'>Try Again</a>");
				return;
			}
